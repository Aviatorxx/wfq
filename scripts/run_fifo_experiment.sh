...
# 清理之前的FIFO结果
rm -f /Users/aviator/Documents/MCP/wfq/results/experiments/*/fifo_*
rm -f /Users/aviator/Documents/MCP/wfq/results/experiments/data/delays_flow_*.csv
...
python3 src/receiver.py \
    --mode echo \
    --port $RECEIVER_PORT \
    --log-file experiments/data/fifo_received_data.log \
    > results/experiments/logs/fifo_receiver.log 2>&1 &
RECEIVER_PID=$!
sleep 2
...
python3 src/router.py \
    ...
    > results/experiments/logs/fifo_router.log 2>&1 &
ROUTER_PID=$!
sleep 2
...
python3 src/sender.py \
    ...
    --log-file results/experiments/logs/fifo_sender1.log \
    > results/experiments/logs/fifo_sender1_output.log 2>&1 &
SENDER1_PID=$!
...
python3 src/sender.py \
    ...
    --log-file results/experiments/logs/fifo_sender2.log \
    > results/experiments/logs/fifo_sender2_output.log 2>&1 &
SENDER2_PID=$!
...
python3 src/sender.py \
    ...
    --log-file results/experiments/logs/fifo_sender3.log \
    > results/experiments/logs/fifo_sender3_output.log 2>&1 &
SENDER3_PID=$!
...
for flow in 1 2 3; do
    if [ -f "delays_flow_${flow}.csv" ]; then
        mv "delays_flow_${flow}.csv" "fifo_delays_flow_${flow}.csv"
        echo "   ✅ delays_flow_${flow}.csv → fifo_delays_flow_${flow}.csv"
    fi
done
...
python3 ../../src/analyze_results.py \
    --algorithm fifo \
    --data-file data/fifo_received_data.log \
...
echo ""
echo "📊 图表文件 (charts/):"
ls -la /Users/aviator/Documents/MCP/wfq/results/experiments/charts/fifo_* 2>/dev/null | sed 's/^/   /' || echo "   (等待图表生成)"
echo ""
echo "📄 数据文件 (data/):"  
ls -la /Users/aviator/Documents/MCP/wfq/results/experiments/data/fifo_* 2>/dev/null | sed 's/^/   /'
echo ""
echo "📋 日志文件 (logs/):"
ls -la /Users/aviator/Documents/MCP/wfq/results/experiments/logs/fifo_* 2>/dev/null | head -3 | sed 's/^/   /'
echo "   ... (更多日志文件)"
echo ""
